...
// Responsive: measures its own width and adapts layout for desktop + mobile.
//
...
const Card = ({ children, style }: any) => (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "16px 18px", ...style }}>{children}</div>
)
...
const Chip = ({ on, onClick, children }: any) => (
    <button onClick={onClick} style={{ padding: "5px 11px", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", border: `1px solid ${on ? C.accent : C.border}`, background: on ? C.accentBg : "transparent", color: on ? C.accent : C.muted }}>{children}</button>
)
function Tile({ label, value, sub, color, bg }: any) {
    return (
        <div style={{ background: bg || C.surface, border: `1px solid ${bg ? (color || C.accent) + "44" : C.border}`, borderRadius: 12, padding: "14px 16px" }}>
            <div style={{ fontSize: 11, color: C.muted, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 8 }}>{label}</div>
            <div style={{ fontSize: 23, fontFamily: "'Sora', sans-serif", fontWeight: 800, color: color || C.text, lineHeight: 1 }}>{value}</div>
        ...
        <div>
            ...
            {DAYS.map((day, di) => (
                <div key={day} style={{ display: "flex", alignItems: "center", gap: 2, marginBottom: 2 }}>
                    <div style={{ width: 26, fontSize: 10, color: C.faint, textAlign: "right", paddingRight: 4, flexShrink: 0 }}>{day}</div>
                    {HOURS.map((h) => <div key={h} title={`${day} ${fmtH(h)}: ${grid[di][h]}`} style={{ flex: 1, aspectRatio: "1", borderRadius: 2, background: heatColor(grid[di][h]), minWidth: 5 }} />)}
                </div>
            ))}
        </div>
    )
...
const MiniStat = ({ label, value, color }: any) => (
    <div>
        <div style={{ fontSize: 10, color: C.faint, textTransform: "uppercase", letterSpacing: "0.05em" }}>{label}</div>
...
export default function SpeedToLead(props: { dataUrl: string }) {
    ...
    const wrap = { background: C.bg, fontFamily: "'DM Sans', sans-serif", color: C.text, padding: mobile ? 14 : 24, borderRadius: 14 } as const
    ...
    const kpiCols = mobile ? "repeat(2, 1fr)" : "repeat(5, 1fr)"

    let body: any = null
    if (!dataUrl || dataUrl.indexOf("YOUR-HOST") >= 0)
        body = <div style={{ color: C.muted, fontSize: 13 }}>Set the <b>Data URL</b> property (right-hand panel) to load the dashboard.</div>
    else if (err) body = <div style={{ color: C.red, fontSize: 13 }}>Couldn’t load data: {err}</div>
    else if (!s || !win) body = <div style={{ color: C.faint, fontSize: 13 }}>Loading…</div>
    else {
        ...
        const header = (
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: mobile ? "stretch" : "flex-start", marginBottom: 16, flexDirection: mobile ? "column" : "row", gap: 12 }}>
                <div>
                    <div style={{ fontSize: mobile ? 19 : 22, fontFamily: "'Sora', sans-serif", fontWeight: 800 }}>Speed to Lead</div>
                    <div style={{ fontSize: 12, color: C.faint, marginTop: 2 }}>{s.meta.location} · manual calls + texts · {bh.open}–{bh.close} {tz}</div>
                </div>
                <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                    {s.windows.map((wd: any) => <Chip key={wd.key} on={win === wd.key} onClick={() => setWin(wd.key)}>{wd.label}</Chip>)}
        ...
        if (rep && repAct[rep]) {
            ...
            const statBox = (bgc: string, lab: string, val: string, valc: string, note?: any) => (
                <div style={{ background: bgc, borderRadius: 10, padding: "11px 13px", marginBottom: mobile ? 0 : 8 }}>
                    <div style={{ fontSize: 11, color: C.muted, textTransform: "uppercase", letterSpacing: "0.05em" }}>{lab}</div>
                    <div style={{ fontSize: 20, fontFamily: "'Sora', sans-serif", fontWeight: 800, color: valc }}>{val}</div>
                    {note}
                </div>
            )
            body = (<>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                    ...
                    <div>
                        <div style={{ fontSize: mobile ? 18 : 20, fontFamily: "'Sora', sans-serif", fontWeight: 800 }}>{rep}</div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: 14, marginBottom: 14 }}>
                    <Card>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12, fontSize: 13 }}><span style={{ color: C.muted }}>Total dials</span><span style={{ fontWeight: 700 }}>{d.dials}</span></div>
                        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6, fontSize: 13 }}><span style={{ color: C.muted }}>Avg call length</span><span style={{ fontWeight: 700 }}>{fmtDur(d.avgCallSec)}</span></div>
                    </Card>
                    <Card>
                ...
                <Card style={{ marginBottom: 14 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "0.9fr 2.1fr", gap: mobile ? 12 : 18, alignItems: "start" }}>
                        <div style={{ display: mobile ? "grid" : "block", gridTemplateColumns: mobile ? "repeat(3, 1fr)" : undefined, gap: mobile ? 8 : 0 }}>
                            {statBox(C.greenBg, "Median (typical)", fmtDur(sp.median), C.green)}
                            {statBox(C.blueBg, "90% within", fmtDur(sp.p90), C.blue)}
                            {statBox(C.bg, "Average", fmtDur(sp.avg), C.faint, skewed ? <div style={{ fontSize: 11, color: C.amber, marginTop: 2 }}>↑ skewed by outliers</div> : null)}
                        </div>
                        <div>
                            ...
                            <div style={{ fontSize: 11, color: C.faint, marginTop: 8 }}>Most leads cluster in the fast buckets; the thin tail on the right pulls the average up — median and the 90% mark ignore it.</div>
                        </div>
                    </div>
            ...
            const repList = (
                <Card>
                    <Label right={<span style={{ fontSize: 11, color: C.accent, fontWeight: 600 }}>tap a rep →</span>}>Rep performance</Label>
                    {mobile ? (
                        a.perRep.map((r: any) => {
                            const clickable = !!repAct[r.rep]
                            const col = colorFor(r.rep)
                            return (
                                <div key={r.rep} onClick={() => clickable && setRep(r.rep)} style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: 12, marginBottom: 8, cursor: clickable ? "pointer" : "default" }}>
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                            <Avatar name={r.rep} color={col} />
                                            <span style={{ fontWeight: 700, fontSize: 14 }}>{r.rep.split(" ")[0]}</span>
                                            {clickable && <span style={{ color: C.faint }}>›</span>}
                                        </div>
                                        <div style={{ textAlign: "right" }}>
                                            <span style={{ fontSize: 11, color: C.faint }}>avg </span>
                                            <span style={{ fontWeight: 800, fontFamily: "'Sora', sans-serif", color: C.accent }}>{fmtDur(r.avg)}</span>
                                        </div>
                                    </div>
                                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginTop: 12 }}>
                                        <MiniStat label="Share" value={fmtPct(r.share)} />
                                        <MiniStat label="Leads" value={fmtNum(r.leads)} />
                                        <MiniStat label="No resp" value={r.noResp || "0"} color={r.noResp ? C.red : C.text} />
                                        <MiniStat label="Median" value={fmtDur(r.median)} color={C.green} />
                                    </div>
                                </div>
                            )
                        })
                ...
                <Card>
                    ...
                    <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                        ...
                        <div style={{ flex: 1, background: C.greenBg, borderRadius: 10, padding: "10px 12px" }}>
                            ...
                            <div style={{ fontSize: 11, color: C.faint }}>typical lead</div>
                        </div>
                    </div>
            ...
            body = (<>
                ...
                <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1.3fr 1fr", gap: 14, marginBottom: 14 }}>
                    {repList}
                    {distCard}
                </div>
                <Card>
                    ...
                    {DAYS.map((day, di) => (
                        <div key={day} style={{ display: "flex", alignItems: "center", gap: 2, marginBottom: 2 }}>
                            <div style={{ width: 26, fontSize: 10, color: C.faint, textAlign: "right", paddingRight: 4, flexShrink: 0 }}>{day}</div>
                            {HOURS.map((h) => <div key={h} title={`${day} ${fmtH(h)}: ${heat[di][h]} leads`} style={{ flex: 1, aspectRatio: "1", borderRadius: 2, background: heatColor(heat[di][h]), minWidth: 5 }} />)}
                        </div>
                    ))}
                </Card>
    ...
    return <div ref={rootRef} style={wrap}>{font}{body}</div>
}
